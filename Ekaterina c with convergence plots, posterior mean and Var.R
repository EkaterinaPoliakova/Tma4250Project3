...
mean_posterior <- colMeans(obs_matrix[1:6,]) #n-dimensional mean vector
#Then ggplot mean_posterior
library(matrixStats)
variance_posterior <-colVars(obs_matrix[1:6,]) #variance of each node
#Then ggplot variance_posterior

mean_posterior.df =data.frame(MMAP=round(mean_posterior),x=xlit,y=ylit)
lab=c("shale","sand")
newplot = #should change the visual look of this
  ggplot(mean_posterior.df, main="Text", aes(x=x, y=y)) + 
  coord_fixed(ratio = 1)+
  theme_minimal() +
  geom_tile(aes(fill=as.factor(MMAP)))+
  scale_fill_manual(name = "Lythology",breaks = waiver(),labels=rev(lab),  values=c(0.1,1.1),  
                    na.value="black")
pdf(paste("MMAP.pdf"), width = 4, height = 4)
print(newplot)
dev.off()

mean_posterior.df =data.frame(Mean=(mean_posterior),x=xlit,y=ylit)
myplot =
  ggplot(mean_posterior.df, aes(x=x, y=y, fill=Mean)) +
  coord_fixed(ratio = 1)+
  theme_minimal() +
  geom_raster() +
  scale_fill_gradient2()
pdf(paste("mean_p.pdf"), width = 4, height = 4)
print(myplot)
dev.off()

var_posterior.df =data.frame(Variance=(variance_posterior),x=xlit,y=ylit)
myplot =
  ggplot(var_posterior.df, aes(x=x, y=y, fill=Variance)) +
  coord_fixed(ratio = 1)+
  theme_minimal() +
  geom_raster() +
  scale_fill_gradient2()
pdf(paste("var_p.pdf"), width = 4, height = 4)
print(myplot)
dev.off()
