...
logbeta <- function(beta) {
  ...
  for (i in (1:n)) {
    ...
    totalsum <- totalsum + log(beta*p_d_l[i,2])*number_neigh[1] + 
      log(beta*p_d_l[i,1]*number_neigh[2])
  }
  return (totalsum)
}

p_d_l <- function(current_l,n,mean_vector,sigma) {
  p_d_l <- matrix(nrow=n,ncol=2)
  for (i in 1:n) {
    p_d_l[i,] = dnorm(current_l[i], mean = mean_vector, sd = sigma)
  }
  return (p_d_l)
}
