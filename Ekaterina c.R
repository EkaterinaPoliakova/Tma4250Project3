...
realizations = replicate(1,(as.integer(rbernoulli(N,prob)))) #N binomial samples with p = prob
...
current_l <-realizations[,1] #initial value in the grid
possible_neigh=Compute_possible_neighours(length(current_l))
n=length(current_l)

#convergence_vector <- replicate(M,0) 
M=50000
for (j in (5028:M)) { #MCMC-sampling (M steps)
  if(j %% 20 ==0) print(paste("Step",j))
 ...
 # convergence_vector[j] <- current_l
